...
export const nodeHttpTransport: Transport = (request) =>
  new Promise<HttpResponse>((resolve, reject) => {
    ...
    try {
      ...
      reject(new PegelNetworkError(`Invalid URL: ${redactUrl(request.url)}`));
      return;
    }
    ...
    if (url.protocol !== "http:" && url.protocol !== "https:") {
      reject(new PegelNetworkError(`Unsupported protocol "${url.protocol}" in URL: ${redactUrl(request.url)}`));
      return;
    }
